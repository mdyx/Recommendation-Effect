...
def i2i(v, j, i_data, iu_data, x, split_cnt):
    ...
    for t in range(l):
        # sort
        ucx = [[user2[t][idx], click2[t][idx], x[user2[t][idx]], t2[t][idx]] for idx in range(len(user2[t]))]
        ucx.sort(key = lambda x: x[2])
        tx = [x for u, c, x, t in ucx]
        ...
        for idx1 in range(len(user1[t])):
            ...
            rec_num1[t] += 1
            click_num1[t] += c1
            tt[t][0].append(t1[t][idx1])
            if len(ucx) > 0:
                rec_num2[t] += 1
                click_num2[t] += ucx[match_idx][1]
                tt[t][1].append(ucx[match_idx][3])
    
    rec_all = [len(user1[t]) + len(user2[t]) for t in range(l)]
    
    return l, rec_num1, click_num1, rec_num2, click_num2, rec_all, tt

def cal_effect(v, j, i_data, iu_data, x, split_cnt):
    l, rec_num1, click_num1, rec_num2, click_num2, rec_all, tt = i2i(v, j, i_data, iu_data, x, split_cnt)
