...
u_rec = []
u_rate = []
c_rate = [[] for cate in range(c_num)]
...
for u in range(len(data)):
    ...
    for cate in range(c_num):
        if c_rec[cate] == 0:
            c_rate[cate].append(0.)
        else:
            c_rate[cate].append(c_click[cate] / c_rec[cate])
...
while test_num < max_test_num:
    ...
    if cnt > 0:
        test_num += 1
        if args.m_model != 'sam':
            x_list.append([[c_rate[i2c[v]][u] for u in u1], [c_rate[i2c[v]][u] for u in u2]])
        # the sam stratifies the samples, there are several subgroups
        else:
            for t in range(len(u1)):
                x_list.append([[c_rate[i2c[v]][u] for u in u1[t]], [c_rate[i2c[v]][u] for u in u2[t]]])
